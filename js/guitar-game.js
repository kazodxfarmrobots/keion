      ...
      function drawGuitarGame(ctx, width, height) {
        ...
        for (let i = 0; i < speedLineCount; i += 1) {
          const sx = ((i * 93 + w.scrollX * (2.4 + speedN * 2.8)) % (width + 180)) - 90;
      ...
      function guitarTick(now) {
        ...
        el.guitarHint.textContent = `RANK NOW: ${rankLabel(nowRank)} / STREAK x${w.streak} / JUMP ${jumpGaugePercent(w)}% (long press)`;
